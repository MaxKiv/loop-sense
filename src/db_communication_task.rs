...
pub async fn communicate_with_db(mut db_receiver: Receiver<SensorData>) {
    ...
    let table_name = initialize(db_client.clone()).await;
    ...
    loop {
        ...
        if let Some(sensor_data) = db_receiver.recv().await {
            ...
            info!("batched_query {:?}", batched_query);
        } else {
        ...
        if batched_query.len() >= QUERY_BATCH_LEN {
            match db_client.query(batched_query.clone()).await {
                ...
                Err(err) => error!(
                    "Error inserting batched measurements into the DB: {:?}",
                    err
                ),
            }
            batched_query.clear();
...
pub async fn initialize(db_client: Arc<Client>) -> TableName {
    ...
    let table_name = format!("sensor_data_{}_{}", date, measurement_id);
